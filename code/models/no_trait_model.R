...
m <- bam(pres ~ arid*wtd*ps + arid2 + wtd2 + ps2 + s(spp, bs = "re") + s(eco, bs = "re"), 
         family = "binomial", data = d, discrete = T)

end <- Sys.time()
print(end-start)

saveRDS(m, "results/models/no_trait_model.rds")
...
saveRDS(m_slim, "results/models/no_trait_model_slim.rds")
